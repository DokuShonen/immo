...
class AuthManager:
    ...
    def login(self, username, password):
        ...
        # 0:id, 1:username, 2:email, 3:password_hash, 4:role, 5:nom, 6:prenom, ...
        if user and self.verify_password(password, user[3]):  # password_hash est bien à l'index 3
            return {
                'id': user[0],
                'username': user[1],
                'email': user[2],
                'role': user[4], # Role est à l'index 4
                'nom': user[5],
                'prenom': user[6],
